...
сть некий репозиторий на github.com, он него нами был сделан форк, добавлены какие-то изменения. Оригинальный (мастер-)репозиторий был как-то обновлён. Задача: стянуть с мастер-репозитория изменения (которые там внесены уже после того, как мы его форкнули).
...
Простое решение: указать git [кешировать ваш пароль.](https://docs.github.com/ru/get-started/getting-started-with-git/caching-your-github-credentials-in-git)
